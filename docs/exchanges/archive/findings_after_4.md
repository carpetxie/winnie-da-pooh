...
**Date:** 2026-02-22
**Status:** Draft — under review (iteration 4).

## Abstract

Prediction market point forecasts and distributional forecasts can diverge dramatically in quality. CPI markets produce accurate point forecasts that beat all benchmarks — yet their distributional spreads actively *harm* forecast quality. We introduce the CRPS/MAE ratio as a diagnostic that flags exactly this decoupling, and apply it to 336 multi-strike Kalshi contracts across 41 economic events.

The answer depends on the series: Jobless Claims distributions robustly add value (CRPS/MAE=0.66, 95% CI [0.57, 0.76]; all 16 leave-one-out ratios below 1.0), while CPI distributions are actively harmful (CRPS/MAE=1.58, 95% CI [1.04, 2.52]; all 14 leave-one-out ratios above 1.0). This heterogeneity is statistically significant (permutation test p<0.001, Mann-Whitney p=0.003 tail-aware, p=0.026 interior-only). Four independent diagnostics converge on CPI miscalibration: CRPS/MAE ratio with CI excluding 1.0, leave-one-out robustness, PIT directional bias (mean PIT=0.61), and a temporal pattern where distributional quality degrades most at mid-life.

In point-forecast comparisons, the CPI implied mean outperforms random walk with a large effect size (d=−0.71, p_adj=0.059 after Bonferroni correction), significant at the 10% level with adequate statistical power (>80%). TIPS breakeven rates Granger-cause Kalshi CPI prices (F=12.2, p=0.005), providing predictive information at 1-day lag.
...
**CPI distributions are harmful.** Using the tail-aware mean, the CI [1.04, 2.52] excludes 1.0, meaning we can reject at 95% confidence the hypothesis that CPI distributional spread is neutral. Leave-one-out analysis confirms robustness: all 14 tail-aware leave-one-out CRPS/MAE ratios remain above 1.0 (range [1.36, 1.78]). No single event drives the CPI finding. The signed difference (CRPS − MAE) is positive for 10 of 14 events (Wilcoxon signed-rank p=0.091) — borderline significant but directionally consistent. Four independent diagnostics converge on CPI miscalibration: (1) the CRPS/MAE ratio exceeds 1.0 with tail-aware CI excluding 1.0; (2) all 14 leave-one-out ratios exceed 1.0; (3) PIT analysis shows systematic directional bias (mean PIT=0.61, consistent with inflation underestimation); and (4) the temporal pattern shows distributional quality is worst at mid-life when traders need it most. The serial-correlation-adjusted CI (ρ=0.23, n_eff≈8.8) is approximately [0.90, 2.58], which includes 1.0 — so the CPI finding is more marginal than the JC finding when accounting for serial dependence. However, the leave-one-out analysis and three additional diagnostics make a strong converging case.

**The CPI–JC divergence is statistically significant.** A permutation test on the ratio-of-means difference (CPI 1.58 vs JC 0.66, difference = 0.92) yields p<0.001 (10,000 permutations), and a Mann-Whitney U test on per-event ratios gives p=0.003 (rank-biserial r=−0.64, large effect) using tail-aware ratios. The result is robust to metric specification: interior-only per-event ratios yield Mann-Whitney p=0.026 (r=−0.48, medium-to-large effect). This confirms the heterogeneity finding is not a sampling artifact and is not driven by the choice of mean specification: CPI and Jobless Claims are drawn from meaningfully different calibration regimes.
...
The calibration gap between CPI and Jobless Claims is not a sampling artifact — it is statistically significant by both permutation test (p<0.001) and Mann-Whitney U (p=0.003). We hypothesize four mechanisms driving this heterogeneity:
...
**The point-vs-distribution decoupling.** This is perhaps the paper's most striking finding for practitioners: CPI point forecasts beat all benchmarks while CPI distributions are actively harmful (CRPS/MAE=1.58, CI excludes 1.0). The market aggregates information into an accurate mean but misprices uncertainty around that mean — accurate centers with miscalibrated spreads. The PIT diagnostic confirms the mechanism: mean PIT=0.61, indicating systematic inflation underestimation that biases the distributional tails without affecting the center. For traders, this means the CPI implied mean is a reliable signal, but the distributional spread should be ignored or replaced with historical dispersion estimates.
...
14. **Formal heterogeneity test**: Permutation test (10,000 permutations) on the CRPS/MAE ratio difference between CPI and JC: p<0.001. Mann-Whitney U test on per-event ratios: p=0.003 (tail-aware, r=−0.64), p=0.026 (interior-only, r=−0.48). Robust across metric specifications
