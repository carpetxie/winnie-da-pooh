...
**Date:** 2026-02-22
**Status:** Draft — under review (iteration 8).
...
**CPI distributions are harmful.** Using the tail-aware mean, the BCa CI [1.04, 2.52] excludes 1.0, meaning we can reject at 95% confidence the hypothesis that CPI distributional spread is neutral. A block bootstrap (block length 2, matching AR(1) structure with ρ=0.23) directly accounts for serial correlation and yields CI [1.06, 2.63], also excluding 1.0. Two independent bootstrap methods — BCa (assuming iid) and block bootstrap (adjusting for serial correlation ρ=0.23) — both exclude 1.0, confirming the finding is not an artifact of either independence or serial-dependence assumptions. (We note the margins are thin: both lower bounds are near 1.0. The case for CPI miscalibration rests not on any single CI but on the convergence of five independent diagnostics below.) (Note: the block bootstrap targets ρ=0.23 from the realized CPI series; the per-event CRPS/MAE ratios themselves show lower autocorrelation — CPI AR(1)≈0.12, JC AR(1)≈0.04 — making this adjustment conservative. The near-zero per-event autocorrelation means distributional quality is approximately independent across events, consistent with event-specific calibration failures rather than persistent structural bias.) Leave-one-out analysis confirms robustness: all 14 tail-aware leave-one-out CRPS/MAE ratios remain above 1.0 (range [1.36, 1.78]). No single event drives the CPI finding. The signed difference (CRPS − MAE) is positive for 10 of 14 events (Wilcoxon signed-rank p=0.091) — borderline significant but directionally consistent. Five independent diagnostics converge on CPI miscalibration: (1) the CRPS/MAE ratio exceeds 1.0 with CI excluding 1.0 under both standard bootstrap and serial-correlation-adjusted block bootstrap; (2) all 14 leave-one-out ratios exceed 1.0; (3) PIT analysis shows systematic directional bias (mean PIT=0.61, consistent with inflation underestimation); (4) the temporal pattern shows distributional quality is worst at mid-life when traders need it most; and (5) the signed difference (CRPS − MAE) is positive for 10/14 events (p=0.091).

**The CPI–JC divergence is statistically significant.** A Mann-Whitney U test on per-event CRPS/MAE ratios gives p=0.003 (rank-biserial r=−0.64, large effect) using tail-aware ratios, and p=0.026 (r=−0.48, medium-to-large effect) using interior-only ratios. A scale-free permutation test on interior-only per-event ratios — which avoids the scale-mixing issue inherent in pooling raw CRPS and MAE values across series with 100,000x scale differences — confirms the result (p=0.016, 10,000 permutations). This confirms the heterogeneity finding is not a sampling artifact, is not driven by the choice of mean specification, and is not an artifact of scale differences: CPI and Jobless Claims are drawn from meaningfully different calibration regimes.
...
**For both series, distributional quality is worst when the outcome is close to the implied mean — and best when the surprise is large.** The CPI events with the highest CRPS/MAE ratios (KXCPI-25JUN: 4.51, KXCPI-25MAR: 3.72) all had small point forecast errors; the events with the lowest ratios (KXCPI-25FEB: 0.35, KXCPI-25AUG: 0.59) had large surprises. This pattern is statistically significant at p<0.01 for CPI.

This finding has two important implications:

1. **The CPI CRPS/MAE > 1 finding is real but nuanced.** The signed-difference test (p=0.091) confirms distributional underperformance independent of any ratio mechanics. But the *severity* of the penalty is concentrated in low-surprise events where the MAE denominator is mechanically small. For large CPI surprises — exactly the scenarios where distributional information is most valuable to traders — the distributions perform comparably to point forecasts.

2. **This is not merely a ratio artifact.** The same inverse pattern appears for Jobless Claims (ρ=−0.55, p=0.029), where CRPS/MAE < 1 at the aggregate level. Even well-calibrated distributions show higher ratios on small surprises. The mechanism is that when the outcome falls near the distribution's center, the sharpness reward (½E|X−X'|) is small relative to the already-small MAE — the distribution's *spread* becomes the dominant cost rather than an asset. This is a general property of CRPS/MAE as a diagnostic: it is most informative for large surprises, where the distributional shape genuinely helps or hurts.

**Conditional CRPS/MAE by surprise direction (CPI only):** Splitting by whether realized CPI exceeded the implied mean: upside inflation surprises (n=9, realized > implied mean) show CRPS/MAE=1.34, while downside surprises (n=5) show CRPS/MAE=2.22. The worse performance on downside surprises reflects the small-MAE mechanical effect (these events had mean MAE=0.038 vs 0.085 for upside), consistent with the Spearman finding rather than a directional asymmetry.

**Strike structure and simulation robustness check:** CPI events average 2.3 evaluated strikes (range 2–3), while Jobless Claims average 2.8 (range 2–5). A Monte Carlo simulation (10,000 trials, three distributional families: Normal, Uniform, Skew-Normal) confirms this difference cannot explain the CRPS gap: **for symmetric distributions, going from 3 to 2 strikes inflates CRPS by ≤2%, far less than the 58% CPI penalty.** The realized CPI data is approximately symmetric (Shapiro-Wilk p=0.24), and the strike-count confound accounts for at most ~5% of the observed gap. (Full simulation details: distributional families matched to each series' realized parameters, piecewise-linear CDFs with 2–5 strikes; for strongly skewed distributions (α=4), the 2→3 strike effect can reach ~49%, but this exceeds the empirical skewness.)
...
Both CPI subsets show CRPS/MAE > 1.0, ruling out the hypothesis that 2-strike coarseness alone explains the CPI penalty. Interestingly, JC 2-strike events show *better* CRPS/MAE (0.46) than 3+-strike events (0.84) — a difference that is statistically significant (Mann-Whitney p=0.028, rank-biserial r=0.66, large effect). This is not merely a selection artifact: it suggests that adding strikes does not automatically improve distributional quality. More strikes create more opportunities for individual contract mispricing, and unless each additional strike attracts sufficient liquidity, the finer resolution can degrade rather than improve the implied distribution. This creates a direct tension with the recommendation below to add CPI strikes — that recommendation is predicated on the new strikes attracting the order flow observed in Jobless Claims markets. Without sufficient liquidity, adding strikes risks replicating the JC 3+-strike degradation pattern.
...
*Note on per-event ratios:* We report per-event ratios using interior-only MAE because per-event tail-aware ratios are unstable: when the tail-aware mean happens to be close to realized by coincidence, the MAE denominator approaches zero and the ratio explodes. For example, KXJOBLESSCLAIMS-26JAN29 has a tail-aware per-event ratio of 10.36 (vs interior-only 1.20) because the tail-aware mean nearly matched the realized value by coincidence (MAE=350 vs CRPS=3,625). Similarly, KXCPI-25JUN: tail-aware MAE=0.01, producing ratio=21.5 vs interior-only 4.51. The aggregate ratio-of-means (which averages numerator and denominator separately before dividing) is immune to this instability — dropping KXJOBLESSCLAIMS-26JAN29 from the JC aggregate gives LOO range [0.64, 0.69], confirming the headline is unaffected.
...
*Temporal hypothesis — three-phase process:* The aggregate CPI temporal pattern (well-calibrated early, worst at mid-life, convergent late) is visible at the median level: per-event median CRPS/MAE trajectories show 0.80 → 1.60 → 0.89 at 10%/50%/90% of market life. Per-event trajectory analysis reveals that 7 of 14 CPI events (50%) individually exhibit the U-shape pattern (10% < 50% > 90%), with the remainder split among monotone-up (2), monotone-down (2), and inverse-U (3). However, the mid-life peak is not individually significant: Wilcoxon signed-rank tests for mid > early (p=0.134) and mid > late (p=0.196) do not reach significance, and early-mid Spearman correlation is near zero (ρ=0.06, p=0.83) — meaning early-life calibration quality does not predict mid-life quality. The U-shape is thus a *population-level tendency* (50% of events) rather than a universal pattern. Reassuringly, zero of 14 mid-life CPI snapshots have CDF monotonicity violations, confirming the CRPS computation is valid. For Jobless Claims, only 4/13 events (31%) show a U-shape, with median trajectory 0.79 → 0.62 → 0.73 — consistently below 1.0 at all timepoints. (For a complementary analysis of how maturity affects individual binary contract accuracy using Brier scores, see Appendix D.)
...
The calibration gap between CPI and Jobless Claims is not a sampling artifact — it is statistically significant by Mann-Whitney U (p=0.003) and scale-free permutation test (p=0.016). We hypothesize four mechanisms driving this heterogeneity:
...
10. **Serial correlation adjustment**: CPI MoM values have AR(1) ρ=0.23, reducing effective degrees of freedom from n=14 to n_eff≈8.8 (Bartlett's formula). This widens the effective CPI CRPS/MAE CI by ~27%. The block bootstrap (item 15) directly addresses this concern and confirms the CI still excludes 1.0
...
12. **Leave-one-out sensitivity**: All 16 Jobless Claims leave-one-out CRPS/MAE ratios fall below 1.0 under both mean specifications (tail-aware range [0.64, 0.69]; interior-only [0.57, 0.66]); all 14 CPI leave-one-out ratios remain above 1.0 (tail-aware [1.36, 1.78]; interior-only [1.17, 1.53]). No single event drives either finding
...
14. **Formal heterogeneity test**: Mann-Whitney U test on per-event ratios: p=0.003 (tail-aware, r=−0.64), p=0.026 (interior-only, r=−0.48). Scale-free permutation test on interior-only per-event ratios: p=0.016 (10,000 permutations). Robust across metric specifications and free of scale-mixing artifacts
15. **Block bootstrap**: Circular block bootstrap (block length 2, matching AR(1) ρ=0.23, 10,000 resamples) for serial-correlation-adjusted CPI CRPS/MAE CI: [1.06, 2.63], excluding 1.0. Eliminates serial correlation as a confound
16. **Cramér-von Mises test**: Supplementary test for PIT uniformity, more powerful than KS for detecting distributional deviations. CPI p=0.152, JC p=0.396 — consistent with KS (both underpowered at current n)
17. **Mid-life CDF monotonicity verification**: All 14 CPI and 13 JC mid-life snapshots used for CRPS computation have monotone CDFs — zero violations. CRPS is computed on valid distributions throughout
18. **Per-event temporal trajectories**: CRPS/MAE computed at 10%, 50%, 90% of market life per event, testing whether the aggregate temporal pattern persists at the individual event level (CPI: 7/14 U-shape, JC: 4/13 U-shape)
...
**CPI:** Neither the KS test (p=0.22) nor the Cramér-von Mises test (p=0.15) rejects PIT uniformity, but both are underpowered at n=14 to detect the observed bias. The CvM test, which is more sensitive to distributional deviations than KS, gives a lower p-value — directionally consistent with the CRPS/MAE finding. The mean PIT CI [0.49, 0.72] includes 0.5, so the directional bias is suggestive rather than individually significant — but it converges with the CRPS/MAE ratio and temporal pattern to form a consistent picture. Few individual PIT values fall in the tails (7% vs ideal 20%), consistent with overconfident (too narrow) distributions.

**Jobless Claims:** Neither the KS test (p=0.35) nor the CvM test (p=0.40) rejects uniformity, and the mean PIT CI [0.35, 0.58] straddles 0.5, consistent with well-calibrated distributions. This directly confirms the CRPS/MAE < 1 finding — the distributions that add value also pass calibration checks, while the distributions that are harmful show directional bias. The contrast provides independent confirmation of the series-level heterogeneity.
